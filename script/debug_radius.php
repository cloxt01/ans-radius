...
define('RADIUS_DB_USER', 'ans_radius');
define('RADIUS_DB_PASS', '95b3783482dc8');
...
// Cek tabel radcheck ada tidak
echo "\n=== TABLE CHECK ===\n";
$tables = ['radcheck', 'radusergroup', 'radreply', 'radgroupreply'];
foreach ($tables as $t) {
    $stmt = $pdo->query("SHOW TABLES LIKE '$t'");
    echo "$t: " . ($stmt->rowCount() > 0 ? "ADA" : "TIDAK ADA") . "\n";
}

// Cek user exists
$username = 'DIANKIANIZA@HARMONI2';
echo "\n=== USER CHECK: $username ===\n";
...
if (empty($rows)) {
    echo "User TIDAK DITEMUKAN di radcheck\n";
    
    // Coba cari mirip (case-insensitive)
    $stmt2 = $pdo->prepare("SELECT username FROM radcheck WHERE LOWER(username) = LOWER(?) LIMIT 5");
    $stmt2->execute([$username]);
    $similar = $stmt2->fetchAll();
    if ($similar) {
        echo "Username mirip ditemukan:\n";
        foreach ($similar as $s) echo "  - " . $s['username'] . "\n";
    }
} else {
    echo "Ditemukan " . count($rows) . " row:\n";
    print_r($rows);
}

// Test fungsi aslinya
echo "\n=== radiusGetUserPassword() ===\n";
$result = radiusGetUserPassword($username);
var_dump($result);

// Cek radiusUserProvisioningReady
echo "\n=== PROVISIONING READY ===\n";
var_dump(radiusUserProvisioningReady());