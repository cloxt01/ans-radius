...
//   php update_isolation_date_fiktif.php           -> dry-run
//   php update_isolation_date_fiktif.php --apply    -> eksekusi update sungguhan
// =====================
$apply = in_array('--apply', $argv);

echo "=================================================\n";
echo "    UPDATE ISOLATION DATE (KHUSUS FIKTIF)        \n";
echo "=================================================\n";
echo $apply
...
// =====================
// AMBIL INVOICE TERAKHIR SETIAP CUSTOMER (HANYA FIKTIF)
// =====================
$sqlInvoices = "
    SELECT 
        i.id,
    ...
    FROM invoices i
    INNER JOIN fiktif_customers fc ON i.customer_id = fc.customer_id -- FILTER KHUSUS FIKTIF
    INNER JOIN (
        SELECT 
            customer_id,
...
if (!$invoices) {
    die("Tidak ada invoice ditemukan untuk pelanggan fiktif.\n");
}
...
echo "\n====================\n";
echo "SELESAI\n";
echo "Total invoice fiktif diproses : " . count($latestPerCustomer) . "\n";
...
// WRITE LOG FILE
// =====================
$logFile = __DIR__ . '/log_isolation_date_fiktif_' . date('Ymd_His') . '.txt';
