...
// Get invoice details
$invoice = fetchOne("SELECT i.*, c.id as customer_id, c.name as customer_name, c.phone as customer_phone, p.name as package_name FROM invoices i LEFT JOIN customers c ON i.customer_id = c.id LEFT JOIN packages p ON c.package_id = p.id WHERE i.id = ? AND i.customer_id = ?", [$invoiceId, $customerId]);
