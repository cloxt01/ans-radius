...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'profile_delete':
            ...
            if ($usageCount > 0) {
                setFlash('error', "Profile Hotspot tidak bisa dihapus karena masih dipakai {$usageCount} user di radusergroup");
                redirect('hotspot.php');
                break;
            }
            $ok = mikrotikDeleteHotspotProfile($id);

            if ($ok) {
                setFlash('success', 'Profile Hotspot berhasil dihapus');
                logActivity('DELETE_HOTSPOT_PROFILE', "ID: $id");
            } else {
                setFlash('error', 'Gagal menghapus profile Hotspot');
            }
            redirect('hotspot.php');
            break;
    }
