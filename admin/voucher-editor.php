        ...
        switch ($_POST['action']) {
            case('save'):
                ...
                if (empty($filename)) {
                    ...
                    $existing = fetchOne("SELECT * FROM settings WHERE setting_key = 'voucher_template'");
                    
                    
                    if ($existing) {
                        update('settings', ['setting_value' => $filename], "setting_key = 'voucher_template'");
                    } else {
                        insert('settings', ['setting_key' => 'voucher_template', 'setting_value' => $filename]);
                    }
                    $message = $existing ? "Template updated to '$filename'." : "Template '$filename' saved.";
                }
                break;
            case('delete'):
                $filename = basename($_POST['filename']);
