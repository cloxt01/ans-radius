...
// Get technicians and agents
$technicians = fetchAll("SELECT * FROM technician_users WHERE status = 'active' ORDER BY name ASC");
$agents = tableExists('agents') ? fetchAll("SELECT * FROM agents WHERE status = 'active' ORDER BY name ASC") : [];
...
if ($customersTableExists) {
    ...
    $selectParts = [
        ...
        'c.isolation_date', 'c.address', 'c.lat', 'c.lng', 'c.status', 'c.created_at', 'c.auto_isolate', 'c.installed_by', 'c.agent_id', 'c.ip_address', 'c.mac_address',
    ...
    <div id="addCustomerModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 2000; align-items: center; justify-content: center;">
        <div class="card" style="width: 920px; max-width: 94%; margin: 2rem; max-height: 92vh; overflow-y: auto; padding: 0;">
            <div style="padding: 20px 25px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; background: var(--bg-card); z-index: 10;">
                ...
                <button type="button" onclick="closeAddCustomerModal()" style="background: none; border: none; color: var(--text-secondary); cursor: pointer; font-size: 1.5rem; padding: 0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;">×</button>
            </div>
...
Paket Wifi : STAR LEGEND" style="background: rgba(255,255,255,0.05); font-family: 'Courier New', monospace; font-size: 0.75rem; color: var(--text-muted);"></textarea>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select name="agent_id" id="add_agent_id" class="form-control" style="color: var(--text-primary); background: var(--bg-card);">
                    ...
                    <div style="display: flex; gap: 10px; margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 20px;">
                        ...
                        <button type="button" class="btn btn-secondary" onclick="closeAddCustomerModal()" style="flex: 1; padding: 12px; font-size: 1rem; font-weight: 500;">
                    ...
                    <tr>
                        ...
                        <td data-label="Nama & Kontak">
                            ...
                            <?php if(!empty($c['agent_name'])): ?>
                                <br><small style="color:var(--neon-orange);"><i class="fas fa-store"></i> <?php echo htmlspecialchars($c['agent_name']); ?></small>
                            <?php endif; ?>
                        </td>
    ...
    <div id="pppoeUserModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 2000;">
        <div class="card" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 90%; max-width: 360px; max-height: 80vh; overflow-y: auto;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                <button type="button" onclick="closePppoeUserModal()" style="background: none; border: none; color: var(--text-secondary); cursor: pointer; font-size: 1.25rem;">×</button>
            </div>
    ...
    <div id="editCustomerModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 2000; align-items: center; justify-content: center;">
        <div class="card" style="width: 800px; max-width: 90%; margin: 2rem; max-height: 90vh; overflow-y: auto; padding: 0;">
            <div style="padding: 20px 25px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; background: var(--bg-card); z-index: 10;">
                ...
                <button type="button" onclick="closeEditModal()" style="background: none; border: none; color: var(--text-secondary); cursor: pointer; font-size: 1.5rem; padding: 0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;">×</button>
            </div>
                ...
                <form method="POST" id="editCustomerForm">
                    ...
                    <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                            ...
                            <div class="form-group">
                                <label class="form-label">Teknisi Instalasi (Opsional)</label>
                                <select name="installed_by" id="edit_installed_by" class="form-control" style="color: var(--text-primary); background: var(--bg-card);">
                                    <option value="">-- Pilih Teknisi --</option>
                                    <?php foreach ($technicians as $tech): ?>
                                        <option value="<?php echo $tech['id']; ?>"><?php echo htmlspecialchars($tech['name']); ?></option>
                                    <?php endforeach; ?>
    ...
    <script>
            ...
            customerTableBody.innerHTML = customers.map(customer => {
                ...
                const radiusBadge = customer.in_radius === true
                    ...
                    : (customer.in_radius === false
                        ...
                        : '');
                const agentHtml = customer.agent_name ? `<br><small style="color:var(--neon-orange);"><i class="fas fa-store"></i> ${escapeHtml(customer.agent_name)}</small>` : '';
        ...
        async function fetchCustomerSearch(search) {
            ...
            if (customerTableBody) {
                customerTableBody.innerHTML = `
            <tr>
                <td colspan="9" style="text-align: center; color: var(--text-muted); padding: 30px;">Memuat...</td>
            </tr>
        `;
            }
            ...
            try {
                ...
                if (data.success && data.data && Array.isArray(data.data.customers)) {
                    ...
                    customerTableBody.innerHTML = `
                <tr>
                    <td colspan="9" style="text-align: center; color: var(--text-muted); padding: 30px;">${escapeHtml(data.message || 'Tidak ada data ditemukan')}</td>
                </tr>
            `;
                }
            } catch (error) {
                ...
                if (customerTableBody) {
                    customerTableBody.innerHTML = `
                <tr>
                    <td colspan="9" style="text-align: center; color: var(--text-muted); padding: 30px;">Terjadi kesalahan saat mencari</td>
                </tr>
            `;
                }
            }
        ...
        function editCustomer(customer) {
            ...
            if (agentSelect) {
                agentSelect.value = customer.agent_id || '';
            }
