...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $id = $_POST['id'];
        $usageCount = function_exists('radiusHotspotProfileUsageCount') ? radiusHotspotProfileUsageCount($id) : 0;
        if ($usageCount > 0) {
            setFlash('error', "Profile tidak bisa dihapus karena masih digunakan oleh {$usageCount} user di radusergroup.");
        } elseif (mikrotikDeleteHotspotProfile($id)) {
...
<div style="background: rgba(0, 170, 255, 0.1); border: 1px solid #2aa7e0; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
    <div style="display: flex; align-items: center; gap: 10px; color: #7ed4ff;">
        <i class="fas fa-cloud" style="font-size: 1.2rem;"></i>
        <div>
            <strong>Hotspot profile disimpan di cloud (Radius DB).</strong>
            <p style="margin: 5px 0 0 0; font-size: 0.9rem; color: #9ee0ff;">
                Tambah, edit, dan hapus profile di halaman ini langsung menulis ke database cloud.
            </p>
        </div>
    </div>
</div>
...
<div class="card">
    ...
    <form method="POST" id="profileForm">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
            ...
            <div class="form-group">
                <label class="form-label">Price (MikroTik Comment)</label>
                            ...
                            <td data-label="Aksi">
                                <div style="display: flex; gap: 5px;">
                                    <button onclick='editProfile(<?php echo json_encode($p); ?>)'
                                        class="btn btn-secondary btn-sm" <?php echo $profileId === '' ? 'disabled style="opacity:.6;cursor:not-allowed;"' : ''; ?>><i class="fas fa-edit"></i></button>
                                    <form method="POST" style="display:inline;" data-no-loading="true" onsubmit="return confirm('Hapus profile ini?')">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="id" value="<?php echo htmlspecialchars($profileId); ?>">
                                        <button type="submit" class="btn btn-danger btn-sm" <?php echo $profileId === '' ? 'disabled style="opacity:.6;cursor:not-allowed;"' : ''; ?>><i
...
<script>
    function editProfile(p) {
        ...
        let price = 0, validity = (p['session-timeout'] || ''), selling = 0;
        if (p['on-login']) {
