...
<div
    ...
    <div class="card" style="margin-bottom: 0;">
        <div style="padding: 15px; display: flex; align-items: center; gap: 15px;">
            <div
                style="width: 45px; height: 45px; border-radius: 10px; background: linear-gradient(135deg, #09d313, #0db4ca); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                <i class="fas fa-server" style="color: #fff; font-size: 1.2rem;"></i>
            </div>
            <div>
                <div style="color: var(--text-primary); font-weight: 600;">
                    IP: <span><?php if (getSettingValue('MIKROTIK_HOST') && mikrotikConnect()) {
                        echo htmlspecialchars(getSettingValue('MIKROTIK_HOST'));
                    } else {
                        echo '<span style="color: var(--text-muted); font-style: italic;">-</span>';
                    } ?></span>
                </div>
                <div style="color: var(--text-muted); font-size: 0.85rem;">User:
                    <?php if (getSettingValue('MIKROTIK_USER')) {
                        echo htmlspecialchars(getSettingValue('MIKROTIK_USER'));
                    } else {
                        echo '<span style="color: var(--text-muted); font-style: italic;">-</span>';
                    } ?>
                </div>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 24px;">
    ...
    <div style="background: linear-gradient(135deg, rgba(0, 245, 255, 0.1), rgba(0, 136, 204, 0.2)); border: 1px solid var(--neon-cyan); border-radius: 16px; padding: 24px; text-align: center; color: var(--text-primary); transition: transform 0.3s, box-shadow 0.3s;"
        ...
        <div style="font-size: 2.8rem; font-weight: 800; color: var(--neon-cyan);"><?php echo $totalCustomers ?? 0 ?></div>
