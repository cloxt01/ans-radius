...
<div class="dashboard-container">
    ...
    <div class="card" style="margin-bottom: 24px;">
        <div class="card-header">
            <div class="card-title"><i class="fas fa-chart-line"></i> Traffic Monitor</div>
            <?php
                $defaultInterface = 'SFP1-INTERNET';
                $selectedName = '';
                if (!empty($interfaces) && is_array($interfaces)) {
                    foreach ($interfaces as $if) {
                        if (strcasecmp(trim($if['name'] ?? ''), $defaultInterface) === 0) {
                            $selectedName = $if['name']; break;
                        }
                    }
                    if ($selectedName === '') $selectedName = $interfaces[0]['name'] ?? '';
                }
            ?>
            <select id="interfaceSelector" class="traffic-selector">
    ...
    <div class="card" style="margin-bottom: 24px;">
        <div class="card-header">
            <div class="card-title"><i class="fas fa-list"></i> PPPoE Log</div>
            <div style="color: var(--text-muted); font-size: 0.85rem;">Showing last <span id="pppoeLimit">20</span> entries</div>
        </div>
        <div class="card-body" style="padding: 12px 16px;">
            <div style="max-height: 255px; overflow-y: auto;">
                <table class="data-table" id="pppoeLogTable" style="table-layout: fixed; width: 100%;">
                    ...
                    <tbody>
                        <tr><td colspan="2" style="text-align:center; padding: 40px; color: var(--text-muted);">Loading PPPoE logs…</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
