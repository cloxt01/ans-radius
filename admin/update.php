...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken((string) $_POST['csrf_token'])) {
        actionLog('UPDATE_CSRF_FAILED', $workdir, "CSRF token tidak valid", json_encode(['ip' => $_SERVER['REMOTE_ADDR']]));
        ...
        actionLog('UPDATE_ACTION_RECEIVED', $workdir, "Menerima aksi update", json_encode(['action' => $action]));
    }
    ...
    if ($action !== '') {
        switch ($action) {
            case 'check':
                actionLog('UPDATE_CHECK_ATTEMPT', $workdir, "Mencoba pengecekan versi update", json_encode(['local_version' => $localVersion ?? 'unknown']));
                ...
                if (empty($urlsToTry)) {
                    ...
                    $statusType = 'error';
                    actionLog('UPDATE_CHECK_FAILED', $workdir, "URL versi update belum dikonfigurasi", json_encode([]));
                } else {
                    ...
                    foreach ($urlsToTry as $url) {
                        ...
                        if ($remoteContent !== false) {
                            ...
                            if (version_compare($localVersion, $remoteVersion, '>=')) {
                                ...
                                $statusType = 'success';
                                actionLog('UPDATE_CHECK_SUCCESS', $workdir, "Versi sudah terbaru", json_encode(['local' => $localVersion, 'remote' => $remoteVersion]));
                            } else {
                                $statusMessage = 'Tersedia versi baru: <strong>' . htmlspecialchars($remoteVersion) . '</strong> (saat ini: ' . htmlspecialchars($localVersion) . ').';
                                $statusType = 'info';
                                actionLog('UPDATE_CHECK_SUCCESS', $workdir, "Versi baru tersedia", json_encode(['local' => $localVersion, 'remote' => $remoteVersion]));
                            }
                            break;
                        }
                    ...
                    if ($statusMessage === '') {
                        ...
                        $statusType = 'error';
                        actionLog('UPDATE_CHECK_FAILED', $workdir, "Gagal mengambil versi dari server", json_encode(['error' => $lastErrorMessage, 'urls' => $urlsToTry]));
                    }
                }
                break;

            case 'update':
                actionLog('UPDATE_ATTEMPT', $workdir, "Mencoba update otomatis via Git", json_encode(['local_version' => $localVersion ?? 'unknown']));
                ...
                if (!$canExec) {
                    ...
                    $returnVar = 1;
                    actionLog('UPDATE_FAILED', $workdir, "exec() tidak tersedia", json_encode([]));
                } elseif (!$isGitRepo) {
                    ...
                    $returnVar = 1;
                    actionLog('UPDATE_FAILED', $workdir, "Bukan repository Git", json_encode(['project_root' => $projectRoot]));
                } else {
                    $statusOut = [];
                    $statusRv = 0;
                    ...
                    if ($statusRv !== 0) {
                        ...
                        $returnVar = 1;
                        actionLog('UPDATE_FAILED', $workdir, "Gagal cek status git", json_encode(['status_rv' => $statusRv, 'output' => $statusOut]));
                        ...
                        $output = array_merge($output, $statusOut);
                        $returnVar = 1;
                        actionLog('UPDATE_FAILED', $workdir, "Ada perubahan lokal", json_encode(['changes' => $statusOut]));
                    } else {
                        exec('cd ' . escapeshellarg($projectRoot) . ' && git pull --ff-only 2>&1', $output, $returnVar);
                        if ($returnVar === 0) {
                            actionLog('UPDATE_SUCCESS', $workdir, "Git pull berhasil", json_encode([]));
                        } else {
                            actionLog('UPDATE_FAILED', $workdir, "Git pull gagal", json_encode(['return_var' => $returnVar, 'output' => $output]));
                        }
                    }
                }
                ...
                if ($returnVar === 0) {
                    ...
                    actionLog('UPDATE_MIGRATION_RUN', $workdir, "Migrasi database dijalankan", json_encode([]));
                }
            ...
            case 'migrate':
                actionLog('MIGRATE_ATTEMPT', $workdir, "Mencoba menjalankan migrasi database", json_encode([]));
                $output = [];
                runDatabaseMigrations($output);
                $statusMessage = implode("\n", $output);
                $statusType = 'success';
                actionLog('MIGRATE_SUCCESS', $workdir, "Migrasi database selesai", json_encode(['output' => $output]));
                break;

            case 'init_git':
                actionLog('INIT_GIT_ATTEMPT', $workdir, "Mencoba inisialisasi Git", json_encode(['project_root' => $projectRoot]));
                $output = [];
                $rv = runGitBootstrapUpdate($projectRoot, $output);
                if ($rv === 0) {
                    actionLog('INIT_GIT_SUCCESS', $workdir, "Inisialisasi Git berhasil", json_encode([]));
                    runDatabaseMigrations($output);
                    actionLog('INIT_GIT_MIGRATION_RUN', $workdir, "Migrasi database setelah init Git dijalankan", json_encode([]));
                } else {
                    actionLog('INIT_GIT_FAILED', $workdir, "Inisialisasi Git gagal", json_encode(['output' => $output, 'return_value' => $rv]));
                }
                $statusMessage = implode("\n", $output);
            ...
            case 'zip_update':
                actionLog('ZIP_UPDATE_ATTEMPT', $workdir, "Mencoba update via ZIP", json_encode([]));
                $output = [];
                $rv = runZipUpdate($projectRoot, $output);
                if ($rv === 0) {
                    actionLog('ZIP_UPDATE_SUCCESS', $workdir, "Update via ZIP berhasil", json_encode([]));
                    runDatabaseMigrations($output);
                    actionLog('ZIP_UPDATE_MIGRATION_RUN', $workdir, "Migrasi database setelah ZIP update dijalankan", json_encode([]));
                } else {
                    actionLog('ZIP_UPDATE_FAILED', $workdir, "Update via ZIP gagal", json_encode(['output' => $output, 'return_value' => $rv]));
                }
                $statusMessage = implode("\n", $output);
                $statusType = $rv === 0 ? 'success' : 'error';
                break;
            ...
            default:
                actionLog('UPDATE_UNKNOWN_ACTION', $workdir, "Aksi tidak dikenali", json_encode(['action' => $action]));
        ...
        if (!isset($statusMessage)) {
            actionLog('UPDATE_NO_ACTION', $workdir, "Tidak ada aksi yang dikirim", json_encode([]));
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <?php if ($statusMessage): ?>
            <div class="alert alert-<?php echo $statusType === 'success' ? 'success' : ($statusType === 'error' ? 'error' : 'info'); ?>" style="white-space: pre-line; width: 100%; height:auto;15px;">
                <p><?php echo htmlspecialchars($statusMessage); ?></p>
            </div>
        <?php endif; ?>
        
        <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 10px; max-width: 400px;">
        <form method="POST" data-no-loading="true" onsubmit="return confirm('Cek versi terbaru dari server update?');">
        ...
        <?php if ($isGitRepo): ?>
            <form method="POST" data-no-loading="true" onsubmit="return confirm('Jalankan git pull untuk update aplikasi?\nPastikan sudah backup terlebih dahulu.');">
        ...
        <form method="POST" data-no-loading="true";  onsubmit="return confirm('Jalankan migrasi database sekarang?\nGunakan ini setelah update manual/ZIP.');">
