    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add':
                ...
                actionLog('ADD_AGENT_ATTEMPT', $workdir, "Mencoba menambahkan agen baru", json_encode(['username' => $username, 'phone' => $phone]));
                ...
                if ($existingUser) {
                    actionLog('ADD_AGENT_FAILED', $workdir, "Username sudah digunakan", json_encode($username));
                ...
                if (!empty($phone)) {
                    ...
                    if ($existingPhone) {
                        actionLog('ADD_AGENT_FAILED', $workdir, "Nomor telepon sudah digunakan", json_encode($phone));
                ...
                if (insert('agents', $data)) {
                    actionLog('ADD_AGENT_SUCCESS', $workdir, "Berhasil menambahkan agen", json_encode($data));
                    setFlash('success', 'Agen berhasil ditambahkan');
                    logActivity('ADD (AGENTS)', "Name: {$data['name']}");
                } else {
                    actionLog('ADD_AGENT_FAILED', $workdir, "Gagal menambahkan agen", json_encode($data));
            ...
            case 'edit':
                $id = (int)$_POST['id'];
                $phone = sanitize($_POST['phone']);
                actionLog('EDIT_AGENT_ATTEMPT', $workdir, "Mencoba memperbarui agen", json_encode(['id' => $id, 'phone' => $phone]));


                if (!empty($phone)) {
                    $existingPhone = fetchOne("SELECT id FROM agents WHERE phone = ? AND id != ?", [$phone, $id]);
                    if ($existingPhone) {
                        actionLog('EDIT_AGENT_FAILED', $workdir, "Nomor telepon sudah digunakan", json_encode($phone));
                        setFlash('error', 'Nomor HP/Telepon sudah terdaftar pada agen lain');
                        redirect('agents.php');
                    }
                }
                ...
                if (update('agents', $data, 'id = ?', [$id])) {
                    actionLog('EDIT_AGENT_SUCCESS', $workdir, "Berhasil memperbarui data agen", json_encode(['id'=>$id, 'data'=>$data]));
                    setFlash('success', 'Data agen berhasil diperbarui');
                    logActivity('UPDATE_AGENT', "ID: {$id}");
                } else {
                    actionLog('EDIT_AGENT_FAILED', $workdir, "Kesalahan tidak terduga", json_encode(['id'=>$id, 'data'=>$data]));
            ...
            case 'delete':
                $id = (int)$_POST['id'];
                actionLog('DELETE_AGEN_ATTEMPT', $workdir, "Mencoba menghapus agen", json_encode($id));
                ...
                if (delete('agents', 'id = ?', [$id])) {
                    ...
                    actionLog('DELETE_AGENT_FAILED', $workdir, "Kesalahan tak terduga", json_encode($id));
