        ...
        switch ($_POST['action']) {
                ...
                if ($customerId) {
                    ...
                    if (!radiusIsUserExistsByUsername($data['pppoe_username'])) {
                        ...
                        if ($ok) {
                            ...
                            actionLog('ADD_RADIUS_FAILED', $workdir, "Gagal menambahkan pelanggan", json_encode(['username' => $data['pppoe_username'], 'password' => $pppoePassword, 'profile' => $profile]));
                        }
                    }
                    ...
                    if ($saveOnu) {
                        try {
                            ...
                            actionLog('UPSERT_ONU_ATTEMPT', $workdir, "Mencoba menambahkan atau mengupdate ONU", json_encode(['odp_id' => $odpId, 'serial' => $serial]));
                            ...
                            if ($exists) {
                                actionLog('UPDATE_ONU', $workdir, "Mencoba mengupdate ONU", json_encode(['serial_number' => $serial, 'payload' => $payload]));
                                update('onu_locations', $payload, 'serial_number = ?', [$serial]);
                            } else {
                                $payload['serial_number'] = $serial;
                                $payload['created_at'] = date('Y-m-d H:i:s');
                                actionLog('INSERT_ONU', $workdir, "Mencoba menambahkan ONU", json_encode(['serial_number' => $serial, 'payload' => $payload]));
                    ...
                    if (!empty($data['installed_by'])) {
                        ...
                        if ($tech && !empty($tech['phone'])) {
                            ...
                            actionLog($waSent ? 'NOTIFY_TECHNICIAN_SUCCESS' : 'NOTIFY_TECHNICIAN_FAILED', $workdir, $waSent ? "Berhasil mengirim notifikasi instalasi ke teknisi" : "Gagal mengirim notifikasi instalasi ke teknisi", json_encode(['customer_id' => $customerId, 'technician_id' => $data['installed_by'], 'phone' => $tech['phone']]));
                        }
                    }
                } else {
                    actionLog('ADD_CUSTOMER_FAILED', $workdir, "Gagal menambahkan pelanggan", json_encode($data));
            ...
            case 'edit':
                ...
                if ($customer_username !== $new_username) {
                    ...
                    actionLog($renameOk ? 'RADIUS_RENAME_SUCCESS' : 'RADIUS_RENAME_FAILED', $workdir, $renameOk ? "Berhasil mengganti username RADIUS" : "Gagal mengganti username RADIUS", json_encode(['old' => $customer_username, 'new' => $new_username]));
                }
                ...
                if ($new_password !== '') {
                    ...
                    if ($old_radius_password !== $new_password) {
                        ...
                        actionLog($pwOk ? 'RADIUS_PASSWORD_UPDATE_SUCCESS' : 'RADIUS_PASSWORD_UPDATE_FAILED', $workdir, $pwOk ? "Berhasil mengupdate password RADIUS" : "Gagal mengupdate password RADIUS", json_encode(['username' => $new_username]));
                    }
                }
                ...
                if (update('customers', $data, 'id = ?', [$customerId])) {
                    ...
                    actionLog('EDIT_CUSTOMER_FAILED', $workdir, "Gagal mengupdate pelanggan", json_encode(['id' => $customerId, 'data' => $data]));
            ...
            case 'delete':
                $customerId = (int)$_POST['customer_id'];
                $customer_username = getPppoeUsernameByCustomerId($customerId);
                actionLog('DELETE_CUSTOMER_ATTEMPT', $workdir, "Mencoba menghapus pelanggan", json_encode(['id' => $customerId, 'username' => $customer_username]));

                if (radiusIsUserExistsByUsername($customer_username)) {
                    actionLog('DELETE_RADIUS_ATTEMPT', $workdir, "Mencoba menghapus user RADIUS & sesi aktif", json_encode(['username' => $customer_username]));
                    radiusDeleteUser($customer_username);
                    mikrotikRemoveActiveSessionByName($customer_username);
                    actionLog('DELETE_RADIUS_SUCCESS', $workdir, "Berhasil menghapus user RADIUS & sesi aktif", json_encode(['username' => $customer_username]));
                }
                if (delete('customers', 'id = ?', [$customerId])) {
                    actionLog('DELETE_CUSTOMER_SUCCESS', $workdir, "Berhasil menghapus pelanggan", json_encode(['id' => $customerId, 'username' => $customer_username]));
                    setFlash('success', 'Pelanggan berhasil dihapus');
                    logActivity('DELETE_CUSTOMER', "ID: {$customerId}");
                } else {
                    actionLog('DELETE_CUSTOMER_FAILED', $workdir, "Gagal menghapus pelanggan", json_encode(['id' => $customerId]));
            ...
            case 'unisolate':
                $customerId = (int)$_POST['customer_id'];
                actionLog('UNISOLATE_CUSTOMER_ATTEMPT', $workdir, "Mencoba membuka isolir pelanggan", json_encode(['id' => $customerId]));
                if (unisolateCustomer($customerId)) {
                    actionLog('UNISOLATE_CUSTOMER_SUCCESS', $workdir, "Berhasil membuka isolir pelanggan", json_encode(['id' => $customerId]));
                    setFlash('success', 'Pelanggan berhasil di-unisolate');
                } else {
                    actionLog('UNISOLATE_CUSTOMER_FAILED', $workdir, "Gagal membuka isolir pelanggan", json_encode(['id' => $customerId]));
            ...
            case 'isolate':
                $customerId = (int)$_POST['customer_id'];
                actionLog('ISOLATE_CUSTOMER_ATTEMPT', $workdir, "Mencoba mengisolir pelanggan", json_encode(['id' => $customerId]));
                if (isolateCustomer($customerId)) {
                    actionLog('ISOLATE_CUSTOMER_SUCCESS', $workdir, "Berhasil mengisolir pelanggan", json_encode(['id' => $customerId]));
                    setFlash('success', 'Pelanggan berhasil diisolir');
                } else {
                    actionLog('ISOLATE_CUSTOMER_FAILED', $workdir, "Gagal mengisolir pelanggan", json_encode(['id' => $customerId]));
            ...
            case 'reset_portal_password':
                ...
                $tempPassword = '1234';
                actionLog('RESET_PORTAL_PASSWORD_ATTEMPT', $workdir, "Mencoba reset password portal pelanggan", json_encode(['id' => $customerId, 'name' => $customer['name']]));

                if (setCustomerPortalPassword($customerId, $tempPassword)) {
                    actionLog('RESET_PORTAL_PASSWORD_SUCCESS', $workdir, "Berhasil reset password portal pelanggan", json_encode(['id' => $customerId, 'name' => $customer['name']]));
                    ...
                    if (trim($phone) !== '') {
                        ...
                        actionLog($waSent ? 'RESET_PORTAL_PASSWORD_WA_SUCCESS' : 'RESET_PORTAL_PASSWORD_WA_FAILED', $workdir, $waSent ? "Notifikasi WhatsApp reset password terkirim" : "Notifikasi WhatsApp reset password gagal terkirim", json_encode(['id' => $customerId, 'phone' => $phone]));
                    }
                    ...
                    actionLog('RESET_PORTAL_PASSWORD_FAILED', $workdir, "Gagal reset password portal pelanggan", json_encode(['id' => $customerId]));
...
<?php if (isset($_SESSION['last_added_customer_id'])): ?>
    <div id="successModal"
        ...
        <div class="card" style="max-width:420px; width:90%; padding:28px; text-align:center;">
            ...
            <p style="color:var(--text-secondary); margin-bottom:20px;">Mau langsung buat pembayaran untuk pelanggan
                ini?</p>
...
<?php endif; ?>
    ...
    <div class="alert alert-warning" style="flex-direction: column;">
        ...
        <p>Pastikan untuk membuat invoice & perpanjang setelah menambahkan pelanggan agar tagihan muncul di portal
            pelanggan dan pelanggan tidak langsung terisolir di hari berikutnya.</p>
    </div>
    ...
    <div id="addCustomerModal"
        ...
        <div class="card"
             style="width: 920px; max-width: 94%; margin: 2rem; max-height: 92vh; overflow-y: auto; padding: 0;">
            <div style="padding: 20px 25px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; background: var(--bg-card); z-index: 10;">
                ...
                <button type="button" onclick="closeAddCustomerModal()"
                        style="background: none; border: none; color: var(--text-secondary); cursor: pointer; font-size: 1.5rem; padding: 0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;">
                    &times;
                </button>
            </div>
            <div style="padding: 25px;">
                <form method="POST" id="addCustomerForm" data-no-loading="true">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="form" id="formInputAdd" class="form-control" rows="10"
                                  placeholder="Nama yang di daftarkan : XXX&#10;Username : XXX@XXX &#10;Password : 1234&#10;Nama wifi : XXXX&#10;Password : XXXX&#10;Alamat : KP XXX&#10;RT/RW : XX/XX&#10;Kecamatan : XXXXX&#10;NO HP : +62 8XXXXXXXX&#10;Paket Wifi : STAR LEGEND"
                                  style="background: rgba(255,255,255,0.05); font-family: 'Courier New', monospace; font-size: 0.75rem; color: var(--text-muted);"></textarea>
                        <button type="button" onclick="loadFormCreateviaAdd()" style="margin-top: 10px;"
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input id="add_name" type="text" name="name" class="form-control" required
                                       placeholder="Nama Lengkap">
                            </div>

                            <div class="form-group">
                                <label class="form-label">Nomor HP (WhatsApp)</label>
                                <input id="add_phone" type="text" name="phone" class="form-control" required
                            ...
                            <div class="form-group-full">
                                ...
                                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                                    <input id="add_pppoe_username" type="text" name="pppoe_username"
                                           id="pppoe_username_input" class="form-control" required
                                           placeholder="Pilih atau ketik username"
                                           style="flex: 1 1 200px; min-width: 0;">
                                    <button type="button" class="btn btn-secondary" onclick="openPppoeUserModal()"
                                            style="flex: 0 0 auto; white-space: nowrap;">Pilih dari Daftar
                                    </button>
                                </div>
                                <small style="color: var(--text-muted);">Pilih username PPPoE dari user MikroTik untuk
                                    menghindari salah input</small>
                            </div>

                            <div class="form-group-full">
                                <label class="form-label">Password PPPoE (Opsional)</label>
                                <input id="add_pppoe_password" type="text" name="pppoe_password" class="form-control"
                                       placeholder="Kosongkan jika tidak ingin mengubah password PPPoE">
                                <small style="color: var(--text-muted);">Jika diisi, password ini akan dikirim ke
                                    perangkat (GenieACS). Aplikasi tidak bisa membaca password dari MikroTik.</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <select id="add_package_id" name="package_id" class="form-control" required
                            ...
                            <div class="form-group">
                                ...
                                <select id="add_router_id" name="router_id" class="form-control" required
                                        style="color: var(--text-primary); background: var(--bg-card);">
                                    <option value="0">Default Router</option>
                                    <?php foreach ($routers as $r): ?>
                                        <option value="<?php echo $r['id']; ?>">
                                            <?php echo htmlspecialchars($r['name']); ?>
                                            (<?php echo htmlspecialchars($r['host']); ?>)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="installed_by" class="form-control"
                                        style="color: var(--text-primary); background: var(--bg-card);">
                            ...
                            <div class="form-group">
                                ...
                                <select name="agent_id" id="add_agent_id" class="form-control"
                                        style="color: var(--text-primary); background: var(--bg-card);">
                            ...
                            <div class="form-group-full">
                                ...
                                <small style="color: var(--text-muted);">Jika dimatikan, pelanggan ini akan diabaikan
                                    oleh isolir otomatis saat tagihan belum dibayar.</small>
                            </div>

                            <div class="form-group-full">
                                <label class="form-label">Alamat</label>
                                <textarea id="add_address" name="address" class="form-control" rows="2"
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" name="lat" id="add_lat" class="form-control" readonly
                                       placeholder="Klik pada peta">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Longitude</label>
                                <input type="text" name="lng" id="add_lng" class="form-control" readonly
                                       placeholder="Klik pada peta">
                            </div>
                        </div>
                        <div id="addMap" class="map-container"></div>
                        <small style="color: var(--text-muted); display: block; margin-top: 8px;">Klik pada peta untuk
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <small style="color: var(--text-muted);">Jika belum ada, tambah ODP di menu GenieACS
                                    Peta</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 10px; margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 20px;">
                        <button type="submit" class="btn btn-primary"
                                style="flex: 1; padding: 12px; font-size: 1rem; font-weight: 500;">
    ...
    <div class="card">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; gap: 12px;">
            <h3 class="card-title" style="margin: 0; display: flex; align-items: center; gap: 8px;"><i
                        class="fas fa-users"></i> Daftar Pelanggan</h3>
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; align-items: center;">
                <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap; justify-content: center;">
                    <select id="filterStatus" class="form-control" style="width: 140px;">
                        ...
                        <option value="active" <?php echo (isset($_GET['filter_status']) && $_GET['filter_status'] === 'active') ? 'selected' : ''; ?>>
                            Aktif
                        </option>
                        <option value="isolated" <?php echo (isset($_GET['filter_status']) && $_GET['filter_status'] === 'isolated') ? 'selected' : ''; ?>>
                            Isolir
                        </option>
                    </select>
                ...
                <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap; justify-content: center; margin-top: 8px;">
                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        ...
                        <input type="date" id="filterLastPaidFrom" class="form-control"
                               value="<?php echo htmlspecialchars($_GET['filter_last_paid_from'] ?? ''); ?>">
                    </div>
                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        <label style="font-size: 0.75rem; color: var(--text-muted);">Last Paid To</label>
                        <input type="date" id="filterLastPaidTo" class="form-control"
                               value="<?php echo htmlspecialchars($_GET['filter_last_paid_to'] ?? ''); ?>">
                    </div>

                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        <label style="font-size: 0.75rem; color: var(--text-muted);">Isolir Dari (Tanggal)</label>
                        <input type="date" id="filterIsolationFrom" class="form-control" style="width: 160px;"
                               value="<?php echo htmlspecialchars($_GET['filter_isolation_from'] ?? ''); ?>">
                    </div>
                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        <label style="font-size: 0.75rem; color: var(--text-muted);">Isolir Sampai (Tanggal)</label>
                        <input type="date" id="filterIsolationTo" class="form-control" style="width: 160px;"
                               value="<?php echo htmlspecialchars($_GET['filter_isolation_to'] ?? ''); ?>">
                    </div>

                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        <label style="font-size: 0.75rem; color: var(--text-muted);">Register From</label>
                        <input type="date" id="filterRegisterFrom" class="form-control"
                               value="<?php echo htmlspecialchars($_GET['filter_register_from'] ?? ''); ?>">
                    </div>
                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        <label style="font-size: 0.75rem; color: var(--text-muted);">Register To</label>
                        <input type="date" id="filterRegisterTo" class="form-control"
                               value="<?php echo htmlspecialchars($_GET['filter_register_to'] ?? ''); ?>">
                    </div>
                </div>

                <div style="display: flex; gap: 12px; justify-content: center; margin-top: 8px;">
                    <button id="applyFilterBtn" class="btn btn-primary"
                            style="padding: 10px 18px; font-size: 1rem; min-width: 110px; border-radius: 8px;">Filter
                    </button>
                    <button id="resetFilterBtn" class="btn btn-secondary"
                            style="padding: 10px 18px; font-size: 1rem; min-width: 110px; border-radius: 8px;">Reset
                    </button>
                </div>
            </div>
        </div>
                ...
                <?php foreach ($customers as $c): ?>
                    <tr>
                        ...
                        <td data-label="Nama & Kontak">
                            ...
                            <small><i class="fab fa-whatsapp"></i> <?php echo htmlspecialchars($c['phone'] ?? 'N/A'); ?>
                            </small>
                        </td>
                        ...
                        <td data-label="PPPoE">
                            <div style="display: flex; align-items: center; gap: 6px;">
                                ...
                                <button type="button"
                                        onclick="copyToClipboard('<?php echo htmlspecialchars($c['pppoe_username']); ?>')"
                                        title="Salin ke clipboard"
                                        style="background: rgba(25, 29, 26, 0.15); border: 1px solid rgba(202, 206, 202, 0.3); color: var(--accent-green); width: 32px; height: 32px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease; font-size: 0.9rem;"
                                        onmouseover="this.style.background='rgba(76,175,80,0.25)'; this.style.borderColor='rgba(7, 7, 7, 0.5)'; this.style.transform='scale(1.05)';"
                                        onmouseout="this.style.background='rgba(76,175,80,0.15)'; this.style.borderColor='rgba(76,175,80,0.3)'; this.style.transform='scale(1);">
                                    <i class="fas fa-copy"></i>
                                </button>
                            </div>
                            <?php if (isset($c['in_radius'])): ?>
                                <?php if ($c['in_radius'] === true): ?>
                                    <span class="badge badge-success" style="margin-left: 5px; margin-top: 5px;"
                                          title="Username terdaftar di RADIUS">
                                    <i class="fas fa-check-circle"></i> OK
                                </span>
                                <?php elseif ($c['in_radius'] === false): ?>
                                    <span class="badge badge-warning" style="margin-left: 5px;"
                                          title="Username TIDAK terdaftar di RADIUS - Timeout tidak akan aktif">
        ...
        <?php if ($totalPages > 1): ?>
            <div id="customerPagination"
                ...
                <a href="?page=1<?php echo $paginationQueryString; ?>"
                   class="btn btn-secondary btn-sm" <?php echo $page === 1 ? 'disabled style="opacity: 0.5;"' : ''; ?>>
                    <i class="fas fa-angle-double-left"></i>
                </a>
                <a href="?page=<?php echo max(1, $page - 1); ?><?php echo $paginationQueryString; ?>"
                   class="btn btn-secondary btn-sm" <?php echo $page === 1 ? 'disabled style="opacity: 0.5;"' : ''; ?>>
            ...
            (Total: <?php echo $totalCustomers; ?> pelanggan)
                ...
                <a href="?page=<?php echo min($totalPages, $page + 1); ?><?php echo $paginationQueryString; ?>"
                   class="btn btn-secondary btn-sm" <?php echo $page === $totalPages ? 'disabled style="opacity: 0.5;"' : ''; ?>>
                    <i class="fas fa-angle-right"></i>
                </a>
                <a href="?page=<?php echo $totalPages; ?><?php echo $paginationQueryString; ?>"
                   class="btn btn-secondary btn-sm" <?php echo $page === $totalPages ? 'disabled style="opacity: 0.5;"' : ''; ?>>
    ...
    <!-- PPPoE User Modal -->
    <div id="pppoeUserModal"
         style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 2000;">
        <div class="card"
             style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 90%; max-width: 360px; max-height: 80vh; overflow-y: auto;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                <h3 style="margin: 0; color: var(--neon-cyan);">
                    <i class="fas fa-network-wired"></i> Pilih Username PPPoE
                </h3>
                <button type="button" onclick="closePppoeUserModal()"
                        style="background: none; border: none; color: var(--text-secondary); cursor: pointer; font-size: 1.25rem;">
                    &times;
                </button>
            </div>
    ...
    <div id="editCustomerModal"
         style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 2000; align-items: center; justify-content: center;">
        <div class="card"
             style="width: 800px; max-width: 90%; margin: 2rem; max-height: 90vh; overflow-y: auto; padding: 0;">
            <div style="padding: 20px 25px; border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; background: var(--bg-card); z-index: 10;">
                <h3 style="margin: 0; color: var(--neon-cyan); font-size: 1.2rem;">
                ...
                <form method="POST" id="editCustomerForm">
                    ...
                    <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                            <div class="form-group">
                                <label class="form-label">Username PPPoE</label>
                                <input type="text" name="pppoe_username" id="edit_pppoe_username" class="form-control"
                                       required placeholder="Username di MikroTik"
                                       style="background: rgba(255,255,255,0.05);">
                            </div>

                            <div class="form-group">
                                <label class="form-label">Password PPPoE</label>
                                <input type="text" name="pppoe_password" id="edit_pppoe_password" class="form-control"
                                       placeholder="(Loading...)" style="background: rgba(255,255,255,0.05);"
                                       autocomplete="off">
                                <small style="color: var(--text-muted); margin-top: 5px; display: block;">Auto-load dari
                                    RADIUS. Kosongkan untuk skip update.</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                            ...
                            <div class="form-group">
                                <label class="form-label">Router / MikroTik</label>
                                <select name="router_id" id="edit_router_id" class="form-control" required
                                        style="color: var(--text-primary); background: var(--bg-card);">
                                    <option value="0">Default Router</option>
                                    <?php foreach ($routers as $r): ?>
                                        <option value="<?php echo $r['id']; ?>">
                            ...
                            <div class="form-group">
                                <label class="form-label">Agen / Reseller (Opsional)</label>
                                <select name="agent_id" id="edit_agent_id" class="form-control"
                                        style="color: var(--text-primary); background: var(--bg-card);">
                                    <option value="">-- Tanpa Agen --</option>
                                    <?php foreach ($agents as $ag): ?>
                                        <option value="<?php echo $ag['id']; ?>"><?php echo htmlspecialchars($ag['name']); ?></option>
                            ...
                            <div class="form-group">
                                ...
                                <small style="color: var(--text-muted);">Jika dimatikan, pelanggan diabaikan saat belum
                                    bayar.</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div class="form-group" style="margin-bottom: 15px;">
                            ...
                            <textarea name="address" id="edit_address" class="form-control" rows="2"
                                      placeholder="Alamat rumah pelanggan" style="resize: vertical;"></textarea>
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 15px;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                <div>
                                    ...
                                    <small style="color: var(--text-muted); display: block; margin-top: 4px;">Klik peta
                                        untuk set</small>
                                </div>
                                <div>
                        ...
                        <div style="height: 300px; margin-top: 15px; border-radius: 8px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1);"
                             id="edit-map-picker"></div>
                    </div>
                    ...
                    <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                            ...
                            <div>
                                ...
                                <select name="odp_id" id="edit_odp_select" class="form-control"
                                        style="color: var(--text-primary); background: var(--bg-card);">
                                    <option value="">-- Pilih ODP --</option>
                                </select>
                            </div>
