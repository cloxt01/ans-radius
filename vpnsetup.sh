...
# =====================================================
# DATABASE CONFIGURATION (Tambahan untuk MySQL Auth)
# =====================================================
# Sesuaikan dengan konfigurasi database MySQL Anda

load_db_config() {
    local CONFIG_FILE="includes/config.php"
    local SEARCH_PATHS=(
        "includes/config.php"
        "../includes/config.php"
        "/var/www/html/includes/config.php"
        "/var/www/includes/config.php"
        "/home/*/public_html/includes/config.php"
    )
    
    echo "Searching for database configuration file..."
    
    # Cari file config.php di berbagai lokasi
    ...
    # Jika file tidak ditemukan, exit dengan error
    if [ ! -f "$CONFIG_FILE" ]; then
        echo "Error: Database configuration file 'includes/config.php' not found!"
        echo "Please make sure the file exists with the following content:"
        echo ""
        echo "<?php"
        echo "// Database Configuration"
        echo "define('DB_HOST', 'localhost');"
        echo "define('DB_NAME', 'ans_radius');"
    ...
    # Extract values from PHP define statements
    DB_HOST=$(grep "define('DB_HOST'" "$CONFIG_FILE" | sed -E "s/.*define\('DB_HOST',\s*'(.*)'.*/\1/" | tr -d "'\"" | head -1)
    DB_NAME=$(grep "define('DB_NAME'" "$CONFIG_FILE" | sed -E "s/.*define\('DB_NAME',\s*'(.*)'.*/\1/" | tr -d "'\"" | head -1)
    DB_USER=$(grep "define('DB_USER'" "$CONFIG_FILE" | sed -E "s/.*define\('DB_USER',\s*'(.*)'.*/\1/" | tr -d "'\"" | head -1)
    DB_PASS=$(grep "define('DB_PASS'" "$CONFIG_FILE" | sed -E "s/.*define\('DB_PASS',\s*'(.*)'.*/\1/" | tr -d "'\"" | head -1)
    
    # Jika tidak ditemukan dengan single quotes, coba double quotes
    if [ -z "$DB_HOST" ]; then
        DB_HOST=$(grep 'define("DB_HOST"' "$CONFIG_FILE" | sed -E 's/.*define\("DB_HOST",\s*"(.*)".*/\1/' | tr -d '"' | head -1)
        DB_NAME=$(grep 'define("DB_NAME"' "$CONFIG_FILE" | sed -E 's/.*define\("DB_NAME",\s*"(.*)".*/\1/' | tr -d '"' | head -1)
        DB_USER=$(grep 'define("DB_USER"' "$CONFIG_FILE" | sed -E 's/.*define\("DB_USER",\s*"(.*)".*/\1/' | tr -d '"' | head -1)
    ...
    # Validasi: pastikan semua nilai terisi (kecuali password boleh kosong)
    if [ -z "$DB_HOST" ] || [ -z "$DB_NAME" ] || [ -z "$DB_USER" ]; then
        echo "Error: Invalid database configuration in $CONFIG_FILE!"
        echo "Please ensure the file contains:"
        echo "  define('DB_HOST', '...');"
        echo "  define('DB_NAME', '...');"
        echo "  define('DB_USER', '...');"
        echo "  define('DB_PASS', '...');"
    ...
    # Export untuk digunakan di fungsi lain
...
# =====================================================
# FUNGSI AUTENTIKASI DATABASE MYSQL
# =====================================================

install_mysql_client() {
	if [ "$USE_DB_AUTH" = 1 ]; then
		echo "Installing MySQL client for database authentication..."
		if [[ "$os" = "debian" || "$os" = "ubuntu" ]]; then
			export DEBIAN_FRONTEND=noninteractive
			(
				set -x
				apt-get -yqq install mysql-client >/dev/null 2>&1
			) || echo "Warning: MySQL client installation failed. DB auth may not work."
		elif [[ "$os" = "centos" || "$os" = "rhel" || "$os" = "fedora" ]]; then
			(
				set -x
				yum -y -q install mysql >/dev/null 2>&1
			) || echo "Warning: MySQL client installation failed. DB auth may not work."
		elif [[ "$os" = "openSUSE" ]]; then
			(
				set -x
				zypper install -y mysql-client >/dev/null 2>&1
			) || echo "Warning: MySQL client installation failed. DB auth may not work."
		fi
	fi
}
...
create_auth_script() {
    if [ "$USE_DB_AUTH" = 1 ]; then
        local AUTH_SCRIPT="/etc/openvpn/server/checkpwd.sh"
        
        echo "========================================="
        echo "Configuring Database Authentication"
        echo "========================================="
        
        # Load database configuration from includes/config.php
        load_db_config
        
        echo "Creating database authentication script at $AUTH_SCRIPT..."
        
        # Gunakan heredoc dengan delimiter yang tidak menggunakan quotes
        # agar variabel bisa diexpansi
        cat > "$AUTH_SCRIPT" << EOF
#!/bin/bash

# OpenVPN MySQL Database Authentication Script
# Generated by OpenVPN installer
...
# Database configuration (loaded from includes/config.php)
DB_HOST="$DB_HOST"
DB_NAME="$DB_NAME"
DB_USER="$DB_USER"
DB_PASS="$DB_PASS"
...
SAFE_USER=\$(echo "\$USERNAME" | sed "s/'//g; s/\"//g; s/;//g")
SAFE_PASS=\$(echo "\$PASSWORD" | sed "s/'//g; s/\"//g; s/;//g")

# Check if username or password is empty
if [ -z "\$SAFE_USER" ] || [ -z "\$SAFE_PASS" ]; then
    exit 1
fi

# Query to check user credentials and status
RESULT=\$(mysql -h "\$DB_HOST" -u "\$DB_USER" -p"\$DB_PASS" -D "\$DB_NAME" -ss -e \
  "SELECT COUNT(*) FROM users 
   WHERE vpn_username='\$SAFE_USER' 
   AND vpn_password='\$SAFE_PASS' 
   AND status='active' 
   AND (expired_at IS NULL OR expired_at > NOW());" 2>/dev/null)
...
if [[ "\$RESULT" =~ ^[0-9]+\$ ]] && [ "\$RESULT" -gt 0 ]; then
    ...
    mysql -h "\$DB_HOST" -u "\$DB_USER" -p"\$DB_PASS" -D "\$DB_NAME" -e \
      "UPDATE users SET last_login=NOW() 
       WHERE vpn_username='\$SAFE_USER';" 2>/dev/null
    exit 0
else
    exit 1
fi
EOF
        ...
        chmod +x "$AUTH_SCRIPT"
        chown nobody:"$group_name" "$AUTH_SCRIPT"
        
        echo "✓ Authentication script created successfully"
        echo "✓ Database configuration from includes/config.php has been applied"
        echo "  - Host: $DB_HOST"
        echo "  - Database: $DB_NAME"
        echo "  - User: $DB_USER"
        echo ""
...
modify_server_config_for_db_auth() {
	if [ "$USE_DB_AUTH" = 1 ]; then
		echo "Modifying OpenVPN configuration for database authentication..."
		
		# Backup original config
		cp "$OVPN_CONF" "${OVPN_CONF}.backup"
		
		# Add authentication configuration
		cat >> "$OVPN_CONF" << 'AUTHCONFIG'
...
AUTHCONFIG
		...
		echo "OpenVPN configuration updated for DB authentication."
	fi
}
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	# Jika menggunakan autentikasi database, tambahkan auth-user-pass
	if [ "$USE_DB_AUTH" = 1 ]; then
		echo "auth-user-pass" >> /etc/openvpn/server/client-common.txt
	fi
}
set_log_permissions() {
	# Set permissions for OpenVPN log files to be readable by web server
	echo "Setting log file permissions for www-data..."
	
	# Cek lokasi file log yang mungkin berbeda
	if [ -f "/var/log/openvpn/openvpn-status.log" ]; then
		chown -R root:www-data /var/log/openvpn/openvpn-status.log
		chmod 644 /var/log/openvpn/openvpn-status.log
		echo "  - Set permissions for /var/log/openvpn/openvpn-status.log"
	fi
	
	if [ -f "/etc/openvpn/server/openvpn-status.log" ]; then
		chown -R root:www-data /etc/openvpn/server/openvpn-status.log
		chmod 644 /etc/openvpn/server/openvpn-status.log
		echo "  - Set permissions for /etc/openvpn/server/openvpn-status.log"
	fi
	
	if [ -f "/var/log/openvpn/ipp.txt" ]; then
		chown -R root:www-data /var/log/openvpn/ipp.txt
		chmod 644 /var/log/openvpn/ipp.txt
		echo "  - Set permissions for /var/log/openvpn/ipp.txt"
	fi
	...
	chown root:www-data /var/log/openvpn
...
finish_setup() {
	...
	echo
	if [ "$USE_DB_AUTH" = 1 ]; then
		echo "Database Authentication is ENABLED."
		echo "  - Users will be authenticated against MySQL database"
		echo "  - Database: $DB_NAME at $DB_HOST"
		echo "  - Make sure the 'users' table exists with required columns"
		echo "  - Client config includes 'auth-user-pass' directive"
	else
		echo "Database Authentication is DISABLED (using certificate auth only)."
	fi
	echo
